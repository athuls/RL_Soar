...
{
	ofstream out_file;
	out_file.open("convert_input_taxi1_multiple_goals.txt");
	/*switch (atoi(argv[1]))
	{
		case 0:
...
//		}*/		
	ifstream in_file;
	in_file.open("input_taxi1_multiple_goals.txt");
	...
	while(!in_file.eof())
	{
		...
		if(s.size()>1)
		{
			if(s.at(0)=="State")
				continue;
			out_file<<map_state[s.at(0)]<<'\t';
			out_file<<s.at(1)<<'\t';
			action=delimit(s.at(2),',');
			for(int i=0;i<action.size()-1;i++)
				out_file<<map_state[action.at(i)]<<",";			
			out_file<<map_state[action.at(action.size()-1)]<<"\t";
			out_file<<s.at(3)<<"\n";
		}
		else if(s.size()==1)
		{	
			action=delimit(s.at(0),'\n'); 
			cout<<map_state[action.at(0)]<<endl;
			out_file<<map_state[action.at(0)]<<endl;
