...
void insertActions(ofstream fileToWrite){
...
int main(int argc, char* argv[])
{
	...
	switch (atoi(argv[1]))
	{
		case 0:
		{	
			mdp_file.open("blocksWorld.txt");
			//Blocks World (Arguments list - 0 <higher probability> <lower probability> <goal state> <initial state>)
			string actions[10]={"1,2,3,4,8,9","0,2,4,6","0,1,3,6","0,2,7,9","0,1,5,8","4,8","1,2","3,9","5,4,9,0","0,3,7,8"};
			//Provide the next state that occurs with highest probability, remaining states would have lesser probability  
			string nextState[10]={"1,2,3,4,8,9","0,2,4,6","0,1,3,6","0,2,7,9","0,1,5,8","4,8","1,2","3,9","5,4,9,0","0,3,7,8"};
			//Number of states in this domain is 10 		
	
			vector<string> split_vec;
			for(int i=0;i<10;i++)
			{
				//Insert next states and probabilities for the next states
				mdp_file<<i<<"\t"<<actions[i]<<"\t";
				...
				for(int k=0;k<nextStates.size();k++)
				{
					if(k!=0)
						mdp_file<<",";	
					if(nextStates.at(k)!=argv[4])	
						mdp_file<<"-0.1:"<<nextStates.at(k);
					else
						mdp_file<<"100:"<<nextStates.at(k);
				}
				mdp_file<<"\n";
			}
			mdp_file<<argv[4]<<endl;
			mdp_file<<argv[5];
			mdp_file.close();
			break;
		}
		case 1:
		{	
			//This is the mines world
			mdp_file.open("minesWorld.txt");
			//actions array contains all the possible actions given the corresponding current state
			string actions[]={"1,5","0,6,2","1,7,3","2,8,4","3,9","0,6,10","1,5,7,11","2,6,8,12","3,7,9,13","4,8,14","5,11,15","6,10,12,16","7,11,13,17","8,12,14,18","9,13,19","10,16,20","11,15,17,21","12,16,18,22","13,17,19,23","14,18,24","15,21","16,20,22","17,21,23","18,22,24","19,23"};
			
			//nextState is an array which gives the next state with highest probability, given the current state and action
			string nextState[]={"1,5","0,6,2","1,7,3","2,8,4","3,9","0,6,10","1,5,7,11","2,6,8,12","3,7,9,13","4,8,14","5,11,15","6,10,12,16","7,11,13,17","8,12,14,18","9,13,19","10,16,20","11,15,17,21","12,16,18,22","13,17,19,23","14,18,24","15,21","16,20,22","17,21,23","18,22,24","19,23"};
			...
			for(int i=0;i<25;i++)
				...
				mdp_file<<"\t";
					...
					if(k!=0)
						mdp_file<<",";
                                        if(contains(nextStates.at(k),tempchar))
						mdp_file<<"-10:"<<nextStates.at(k);
					else if(!nextStates.at(k).compare(argv[5]))
                                                mdp_file<<"100:"<<nextStates.at(k);
                                        else
                                                mdp_file<<"-0.1:"<<nextStates.at(k);
                                }
