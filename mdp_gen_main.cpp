...
int main(int argc, char* argv[])
{
	...
	switch (atoi(argv[1]))
	{
		...
		case 2:	//arguments - case(2), initial state of car, initial state of passenger, goal state, fuelgrid, levels of fuel, pick/drop locations 
		{
			...
			in_file<<argv[4]<<"-"<<"1"<<"-"<<"0"<<"-"<<argv[4]<<endl;
			in_file<<argv[4]<<"-"<<"2"<<"-"<<"0"<<"-"<<argv[4]<<endl;
			in_file<<argv[4]<<"-"<<"3"<<"-"<<"0"<<"-"<<argv[4]<<endl;
			in_file<<argv[4]<<"-"<<"4"<<"-"<<"0"<<"-"<<argv[4]<<endl;
			in_file<<argv[4]<<"-"<<"5"<<"-"<<"0"<<"-"<<argv[4]<<endl;
			break;
		}
	}	
	in_file.close();	
